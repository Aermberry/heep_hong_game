...
class StrokeRendererBase {
  ...
  _renderPath(path) {
    let pathStr = `M ${path[0].x} ${path[0].y} L`;
    path.slice(1).forEach((item) => {
      pathStr += ` ${item.x} ${item.y}`
    })
    return pathStr;
  }
...
class StrokeRenderer extends StrokeRendererBase {
  ...
  mount(target) {
    ...
    if(this._type != 2) {
      ...
      attr(this._animationPath, 'd', this._renderPath(this.stroke.points))
    }
    attrs(this._animationPath, {
      stroke: '#FFFFFF',
      'stroke-width': this._type != 2 ?  STROKE_WIDTH.toString() : 15,
      fill: 'none',
      'stroke-linecap': 'round',
      'stroke-linejoin': 'miter',
      'stroke-dasharray': this._type != 2 ? `${this._pathLength},${this._pathLength}` : '30 25'
    });
    if(this._type == 2) {
      attr(this._animationPath, 'marker-end', 'url(#arrow)')
    }
...
class CharacterRenderer {
  // type 0 mainChar 1 outlineChar; 2 highlightChar
  constructor(character, type) {
    this._oldProps = undefined;
    this._type = type;
    this._strokeRenderers = character.strokes.map(stroke => new StrokeRenderer(stroke, this._type ));
  }
    ...
    if (colorsChanged || strokes !== ((_this$_oldProps3 = this._oldProps) === null || _this$_oldProps3 === void 0 ? void 0 : _this$_oldProps3.strokes)) {
      for (let i = 0; i < this._strokeRenderers.length; i++) {
        ...
        if (this._type >= 1) {
          ...
          this._strokeRenderers[i].render({
            strokeColor: strokeColorArr[`${i<=3 ? i : i % 4 }`],
            opacity: strokes[i].opacity,
            displayPortion: strokes[i].displayPortion
          });
        }
...
class HanziWriterRenderer {
  constructor(character, positioner) {
    this._character = character;
    this._positioner = positioner;
    this._mainCharRenderer = new CharacterRenderer(character, 0);
    //渲染代描外形
    this._outlineCharRenderer = new CharacterRenderer(character, 1);
    this._highlightCharRenderer = new CharacterRenderer(character, 2);
